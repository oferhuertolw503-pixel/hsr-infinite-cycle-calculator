"""CLI handlers that run analyses and render their terminal output."""
...
def run_single(path, audit=False, sensitivity=False, repair=False):
...
def run_search(path, team_size=None, max_rounds=None, top=10):
    data = load_example_json(path)
    character_paths = data.get("characters") or data.get("team")
    if not character_paths:
        raise ValueError(f"{path} needs a 'characters' (or legacy 'team') list")
